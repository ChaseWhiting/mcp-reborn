...
public class RaidCommand {
    ...
    private static int makeRaidersGlow(CommandSource source) throws CommandSyntaxException {
        Raid raid = getRaid(getPlayer(source));
        if (raid != null) {
            for (AbstractRaiderEntity raider : raid.getAllRaiders()) {
    ...
    private static int spawnRaidLeader(CommandSource source) {
        ...
        AbstractRaiderEntity raider = EntityType.PILLAGER.create(source.getEntity().getCommandSenderWorld());
        raider.setPatrolLeader(true);
        raider.setItemSlot(EquipmentSlotType.HEAD, Raid.getLeaderBannerInstance());
        raider.setPos(source.getPosition().x(), source.getPosition().y(), source.getPosition().z());
        raider.finalizeSpawn((IServerWorld) source.getEntity().getCommandSenderWorld(), source.getEntity().getCommandSenderWorld().getCurrentDifficultyAt(new BlockPos(source.getPosition())), SpawnReason.COMMAND, (ILivingEntityData) null, (CompoundNBT) null);
        source.getEntity().getCommandSenderWorld().addFreshEntity(raider);
        return 1;
    }
    ...
    private static int startRaid(CommandSource source, int omenLevel) throws CommandSyntaxException {
        ...
        RaidManager raidManager = serverWorld.getRaids();
        Raid raid = raidManager.getRaidAt(blockPos);
        if (raid != null) {
    ...
    private static int checkRaid(CommandSource source) throws CommandSyntaxException {
        ...
        if (raid != null) {
            ...
            message.append(raid.getTotalRaidersAlive());
            message.append(" Raid health: ");
            message.append(raid.getHealthOfLivingRaiders());
            message.append(" / ");
            message.append(raid.getTotalRaidersAlive());
            source.sendSuccess(new StringTextComponent(message.toString()), false);
            return 1;
    ...
    private static Raid getRaid(ServerPlayerEntity player) {
        ServerWorld serverWorld = (ServerWorld) player.getCommandSenderWorld();
        RaidManager raidManager = serverWorld.getRaids();
        return raidManager.getRaidAt(new BlockPos(player.getX(), player.getY(), player.getZ()));
    }

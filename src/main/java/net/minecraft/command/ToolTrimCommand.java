...
public class ToolTrimCommand {

    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(
                Commands.literal("tooltrim")
                        .requires(cs -> cs.hasPermission(2))
                        .then(Commands.argument("targets", EntityArgument.players())
                                .then(Commands.argument("item", ItemArgument.item())
                                        .then(Commands.argument("pattern", ResourceLocationArgument.id())
                                                .suggests(SuggestionProviders.TOOL_TRIM_PATTERNS)
    ...
    private static int giveTrimmedArmor(CommandContext<CommandSource> ctx, int count) throws CommandSyntaxException {
        ...
        if (!ToolTrim.TRIMMABLE_TOOLS.contains(itemInput.getItem())) {
            ctx.getSource().sendFailure(new StringTextComponent("Item is not a valid armor piece for trim!"));
            return 0;
        }

        ToolTrimPattern pattern = Registry.TOOL_TRIM_PATTERN.get(patternRL);
        ...
        for (ServerPlayerEntity player : targets) {
            ...
            while (remaining > 0) {
                ...
                ToolTrim.setTrim(null, stack, new ToolTrim(material, pattern));

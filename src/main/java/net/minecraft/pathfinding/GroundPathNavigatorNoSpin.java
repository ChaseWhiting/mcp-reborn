...
public class GroundPathNavigatorNoSpin extends GroundPathNavigator {
    private float distancemodifier = 0.75F;

    public GroundPathNavigatorNoSpin(Mob entitylivingIn, World worldIn) {
        super(entitylivingIn, worldIn);
    }

    public GroundPathNavigatorNoSpin(Mob entitylivingIn, World worldIn, float distancemodifier) {
        super(entitylivingIn, worldIn);
    ...
    protected void followThePath() {
        ...
        if (flag || mob.canCutCorner(this.path.getNextNode().type) && this.shouldTargetNextNodeInDirection(vector3d)) {
            this.path.advance();
        }

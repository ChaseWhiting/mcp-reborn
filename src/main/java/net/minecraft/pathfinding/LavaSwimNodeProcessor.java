...
public class LavaSwimNodeProcessor extends NodeProcessor {
   private final boolean allowBreaching;

   public LavaSwimNodeProcessor(boolean p_i48927_1_) {
   ...
   public PathNodeType getBlockPathType(IBlockReader p_186330_1_, int p_186330_2_, int p_186330_3_, int p_186330_4_) {
      ...
      if (fluidstate.isEmpty() && blockstate.isPathfindable(p_186330_1_, blockpos.below(), PathType.LAVA) && blockstate.isAir()) {
         return PathNodeType.BREACH;
      } else {
         return fluidstate.is(FluidTags.LAVA) || fluidstate.is(FluidTags.LAVA) && blockstate.isPathfindable(p_186330_1_, blockpos, PathType.LAVA) ? PathNodeType.LAVA : PathNodeType.BLOCKED;
      }
   }
   ...
   private PathPoint getWaterNode(int p_186328_1_, int p_186328_2_, int p_186328_3_) {
      ...
      if ((!this.allowBreaching && pathnodetype == PathNodeType.BREACH) || pathnodetype == PathNodeType.BLOCKED) {
         return null;
      }
   ...
   protected PathPoint getNode(int p_176159_1_, int p_176159_2_, int p_176159_3_) {
      ...
      if (f >= 0.0F) {
         ...
         if (!this.level.getFluidState(new BlockPos(p_176159_1_, p_176159_2_, p_176159_3_)).is(FluidTags.LAVA)) {
   ...
   private PathNodeType isFree(int p_186327_1_, int p_186327_2_, int p_186327_3_) {
      ...
      for(int i = p_186327_1_; i < p_186327_1_ + this.entityWidth; ++i) {
         for(int j = p_186327_2_; j < p_186327_2_ + this.entityHeight; ++j) {
            for(int k = p_186327_3_; k < p_186327_3_ + this.entityDepth; ++k) {
               ...
               if (fluidstate.isEmpty() && blockstate.isPathfindable(this.level, blockpos$mutable.below(), PathType.LAVA) && blockstate.isAir()) {
                  return PathNodeType.BREACH;
               }

               if (!fluidstate.is(FluidTags.LAVA) && !fluidstate.is(FluidTags.LAVA)) {
      ...
      return blockstate1.isPathfindable(this.level, blockpos$mutable, PathType.LAVA) ? PathNodeType.LAVA : PathNodeType.BLOCKED;
   }
}
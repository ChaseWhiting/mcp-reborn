...
public class FireBlock extends AbstractFireBlock {
    ...
    private final Object2IntMap<Block> VERY_HARDMODE_FLAME_ODDS = Util.make(new Object2IntOpenHashMap<>(), map -> {
        map.put(Blocks.CHEST, 5);
        map.put(Blocks.BARREL, 5);
        map.put(Blocks.TORCH, 30);
        map.put(Blocks.WALL_TORCH, 30);
        map.put(Blocks.REDSTONE_WIRE, 12);
        map.put(Blocks.REDSTONE_BLOCK, 20);
    });

    private final Object2IntMap<Block> VERY_HARDMODE_BURN_ODDS = Util.make(new Object2IntOpenHashMap<>(), map -> {
        map.put(Blocks.CHEST, 50);
        map.put(Blocks.BARREL, 45);
        map.put(Blocks.TORCH, 100);
        map.put(Blocks.WALL_TORCH, 100);
        map.put(Blocks.REDSTONE_WIRE, 70);
        map.put(Blocks.REDSTONE_BLOCK, 80);
    });
    ...
    private int getBurnOdd(BlockState bs) {
        return bs.hasProperty(BlockStateProperties.WATERLOGGED) && bs.getValue(BlockStateProperties.WATERLOGGED) ? 0 : this.burnOdds.getInt(bs.getBlock());
    }

    private int getFlameOdds(BlockState bs) {
        if (bs.hasProperty(BlockStateProperties.WATERLOGGED) && bs.getValue(BlockStateProperties.WATERLOGGED)) {
            return 0;
        }


        return this.flameOdds.getInt(bs.getBlock());
    }

    private boolean checkForCopper(BlockPos bp, World world, Random random) {
        if (world.getBlockState(bp).is(BlockTags.COPPER_THAT_CAN_BURN)) {
            if (world instanceof ServerWorld) {
                WeatheringCopper weatheringCopper = (WeatheringCopper) world.getBlockState(bp).getBlock();
                weatheringCopper.changeOverTime(world.getBlockState(bp), ((ServerWorld) world), bp, random);
            }
    ...
    private void checkBurnOut(World world, BlockPos bp, int chance, Random random, int age) {
        int i = this.getBurnOdd(world.getBlockState(bp));
        if (random.nextInt(chance) < i) {
            BlockState blockstate = world.getBlockState(bp);
            if (random.nextInt(age + 10) < 5 && !world.isRainingAt(bp)) {
                int j = Math.min(age + random.nextInt(5) / 4, 15);
                if (!checkForCopper(bp, world, random)) {
                    world.setBlock(bp, this.getStateWithAge(world, bp, j), 3);
    ...
    public static void bootStrap() {
        ...
        fireblock.setFlammable(Blocks.CHEST, 50, 100);
        fireblock.setFlammable(Blocks.BARREL, 45, 100);
        fireblock.setFlammable(Blocks.TORCH, 30, 100);
        fireblock.setFlammable(Blocks.WALL_TORCH, 30, 100);
        fireblock.setFlammable(Blocks.REDSTONE_WIRE, 12, 70);
        fireblock.setFlammable(Blocks.REDSTONE_BLOCK, 20, 80);

        for (Block block : Registry.BLOCK) {
            if (block instanceof WeatheringCopper weatheringCopper && !(block instanceof WeatheringCopperButtonBlock)) {
                if (weatheringCopper.getNext(block.defaultBlockState()).isPresent()) {
                    fireblock.setFlammable(block, 3, 100);
                }
            }
        }
        ...
        for (Block block : Registry.BLOCK) {
            if (block instanceof AbstractGlassBlock || block instanceof PaneBlock) {
                fireblock.setFlammable(block, 20, 10);
            }
        }

...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
   ...
   public void handleAddEntity(SSpawnObjectPacket p_147235_1_) {
      ...
      } else if (entitytype == EntityType.GRAPPLING_HOOK_ENTITY) {
         Entity entity1 = this.level.getEntity(p_147235_1_.getData());
         if (entity1 instanceof PlayerEntity) {
            entity = new GrapplingHookEntity((PlayerEntity) entity1, this.level);
         } else {
            entity = null;
         }
      } else {
   ...
   public void handleCustomPayload(SCustomPayloadPlayPacket p_147240_1_) {
      ...
      try {
         ...
         if (SCustomPayloadPlayPacket.BRAND.equals(resourcelocation)) {
           ...
           // System.out.println("Received custom payload packet: DEBUG_PATHFINDING_PACKET");
            int entityId = packetbuffer.readInt();
            float maxDistance = packetbuffer.readFloat();
            Path path = Path.createFromStream(packetbuffer);
           // System.out.println("Reading packet of: " + packetbuffer);
            this.minecraft.debugRenderer.pathfindingRenderer.addPath(entityId, path, maxDistance);
            ...
            int entityId = packetbuffer.readInt();
            int numberOfGoals = packetbuffer.readInt();
            List<EntityAIDebugRenderer.Entry> goals = Lists.newArrayList();

            for (int i = 0; i < numberOfGoals; ++i) {
               int priority = packetbuffer.readInt();
               boolean isActive = packetbuffer.readBoolean();
               String description = packetbuffer.readUtf(255);
               goals.add(new EntityAIDebugRenderer.Entry(blockpos6, priority, description, isActive));
            }

            this.minecraft.debugRenderer.goalSelectorRenderer.addGoalSelector(entityId, goals);
            ...
            //System.out.println("Reading packet of: " + packetbuffer + " raid centers: " + collection);
            ...
            IPosition position = new Position(x, y, z);
            BlockPos targetPos = packetbuffer.readBlockPos();
            UUID uuid = packetbuffer.readUUID();
            int entityId = packetbuffer.readInt();
            boolean hasHomePos = packetbuffer.readBoolean();
            BlockPos homePos = hasHomePos ? packetbuffer.readBlockPos() : null;
            boolean isLeader = packetbuffer.readBoolean();

            int homeMemberCount = packetbuffer.readInt();
            List<UUID> homeMembers = Lists.newArrayList();
            for (int i = 0; i < homeMemberCount; i++) {
               homeMembers.add(packetbuffer.readUUID());
            }

            int hunger = packetbuffer.readInt();
            int thirst = packetbuffer.readInt();
            ...
            int entityId = packetbuffer.readInt();
            System.out.println("Received mob ID: " + entityId);
            boolean canReach = packetbuffer.readBoolean();
            System.out.println("Received canReach: " + canReach);
            int nextNodeIndex = packetbuffer.readInt();
            System.out.println("Received nextNodeIndex: " + nextNodeIndex);

            int targetNodesCount = packetbuffer.readInt();
            System.out.println("Received targetNodesCount: " + targetNodesCount);
            ...
            for (int i = 0; i < targetNodesCount; i++) {
               ...
               FlaggedPathPoint lastPoint = targetNodes.iterator().next();
               System.out.println("Received FlaggedPathPoint: " + lastPoint.x + ", " + lastPoint.y + ", " + lastPoint.z);
            }

            int targetPosX = packetbuffer.readInt();
            int targetPosY = packetbuffer.readInt();
            int targetPosZ = packetbuffer.readInt();
            System.out.println("Received targetPos: " + targetPosX + ", " + targetPosY + ", " + targetPosZ);

            int nodeCount = packetbuffer.readInt();
            System.out.println("Received node count: " + nodeCount);
            List<PathPoint> pathPoints = Lists.newArrayList();
            for (int i = 0; i < nodeCount; i++) {
               pathPoints.add(PathPoint.createFromStream(packetbuffer));
               System.out.println("Received PathPoint: " + pathPoints.get(i).x + ", " + pathPoints.get(i).y + ", " + pathPoints.get(i).z);
            }

            int openSetLength = packetbuffer.readInt();
            System.out.println("Received openSet length: " + openSetLength);
            PathPoint[] openSet = new PathPoint[openSetLength];
            for (int i = 0; i < openSetLength; i++) {
               openSet[i] = PathPoint.createFromStream(packetbuffer);
               System.out.println("Received openSet PathPoint: " + openSet[i].x + ", " + openSet[i].y + ", " + openSet[i].z);
            }

            int closedSetLength = packetbuffer.readInt();
            System.out.println("Received closedSet length: " + closedSetLength);
            PathPoint[] closedSet = new PathPoint[closedSetLength];
            for (int i = 0; i < closedSetLength; i++) {
               closedSet[i] = PathPoint.createFromStream(packetbuffer);
               System.out.println("Received closedSet PathPoint: " + closedSet[i].x + ", " + closedSet[i].y + ", " + closedSet[i].z);
            }

            Path path = new Path(pathPoints, new BlockPos(targetPosX, targetPosY, targetPosZ), canReach);
            path.setNextNodeIndex(nextNodeIndex);
            path.targetNodes = targetNodes;
            path.openSet = openSet;
            path.closedSet = closedSet;

            PathDebugRenderer.PathInfo pathInfo = new PathDebugRenderer.PathInfo(entityId, pathPoints);
            this.minecraft.debugRenderer.pathDebugRenderer.addOrUpdatePathsCalculation(pathInfo);
         } else {

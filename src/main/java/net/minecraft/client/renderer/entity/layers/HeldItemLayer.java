...
public class HeldItemLayer<T extends LivingEntity, M extends EntityModel<T> & IHasArm> extends LayerRenderer<T, M> {
   ...
   private void renderArmWithItem(LivingEntity p_229135_1_, ItemStack p_229135_2_, ItemCameraTransforms.TransformType p_229135_3_, HandSide p_229135_4_, MatrixStack p_229135_5_, IRenderTypeBuffer p_229135_6_, int p_229135_7_) {
      if (p_229135_2_.is(Items.SPYGLASS) && p_229135_1_.getUseItem() == p_229135_2_ && p_229135_1_.swingTime == 0) {
         renderArmWithSpyglass(p_229135_1_, p_229135_2_, p_229135_4_, p_229135_5_, p_229135_6_, p_229135_7_);
         return;
      }
   ...
   private void renderArmWithSpyglass(LivingEntity livingEntity, ItemStack itemStack, HandSide humanoidArm, MatrixStack poseStack, IRenderTypeBuffer multiBufferSource, int n) {
       if (livingEntity instanceof PlayerEntity) {
           poseStack.pushPose();
           ModelPart modelPart = ((HumanoidModel<?>)this.getParentModel()).getHead();
           float f = modelPart.xRot;
           modelPart.xRot = MathHelper.clamp(modelPart.xRot, -0.5235988f, 1.5707964f);
           modelPart.translateAndRotate(poseStack);
           modelPart.xRot = f;
           translateToHead(poseStack, false);
           boolean bl = humanoidArm == HandSide.LEFT;
           poseStack.translate((bl ? -2.5f : 2.5f) / 16.0f, -0.0625, 0.0);
           Minecraft.getInstance().getItemInHandRenderer().renderItem(livingEntity, itemStack, ItemCameraTransforms.TransformType.HEAD, false, poseStack, multiBufferSource, n);
           poseStack.popPose();
       } else {
           poseStack.pushPose();
           ModelRenderer modelPart = ((BipedModel<?>)this.getParentModel()).getHead();
           float f = modelPart.xRot;
           modelPart.xRot = MathHelper.clamp(modelPart.xRot, -0.5235988f, 1.5707964f);
           modelPart.translateAndRotate(poseStack);
           modelPart.xRot = f;
   ...
   public static void translateToHead(MatrixStack poseStack, boolean bl) {
      float f = 0.625f;

...
public class DebugRenderer {
   ...
   public RaccoonDebugRenderer raccoonDebugRenderer;
   public PathDebugRenderer pathDebugRenderer;

   private boolean renderChunkborder;

   public static boolean isDebugging = DebugUtils.isDebugging();
   ...
   public DebugRenderer(Minecraft p_i46557_1_) {
      ...
      this.raccoonDebugRenderer = new RaccoonDebugRenderer(p_i46557_1_);
      ...
      this.pathDebugRenderer = new PathDebugRenderer(p_i46557_1_);
   }
   ...
   public void clear() {
      ...
      this.raccoonDebugRenderer.clear();
      this.pathDebugRenderer.clear();
   }
   ...
   public void render(MatrixStack matrixStack, IRenderTypeBuffer.Impl buffer, double cameraX, double cameraY, double cameraZ) {
      if (this.renderChunkborder && !Minecraft.getInstance().showOnlyReducedInfo()) {
         this.chunkBorderRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
      }

      boolean flag = !Minecraft.getInstance().showOnlyReducedInfo();
      if (DebugCommand.shouldRenderAll()) {
         if (isDebugging) {
            if (!DebugCommand.shouldRenderAll()) {
               if (DebugCommand.isDebugPathEnabled() == false) {
                  this.pathfindingRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
               }
               ...
               if (DebugCommand.isDebugRaidEnabled() == false) {
                  this.raidDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
               }
               if (DebugCommand.isDebugBeeEnabled() == false) {
                  this.beeDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
               }
               if (DebugCommand.isDebugBrainAIEnabled() == false) {
                  this.brainDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
               }
            }
         } else {
            this.pathDebugRenderer.render(matrixStack,buffer,cameraX,cameraY,cameraZ);
            if (flag && DebugCommand.isDebugPathEnabled()) {
               this.pathfindingRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
            }
         ...
         if (flag && DebugCommand.isDebugRaidEnabled()) {
            this.raidDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
         }
         if (flag && DebugCommand.isDebugBeeEnabled()) {
            this.beeDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
         }
         if (flag && DebugCommand.isDebugBrainAIEnabled()) {
            this.brainDebugRenderer.render(matrixStack, buffer, cameraX, cameraY, cameraZ);
         }
      }
   }

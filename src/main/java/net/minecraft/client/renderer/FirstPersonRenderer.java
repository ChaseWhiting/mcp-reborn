...
public class FirstPersonRenderer {
    ...
    private void renderMapHand(MatrixStack p_228403_1_, IRenderTypeBuffer p_228403_2_, int p_228403_3_, HandSide p_228403_4_) {
        ...
        NewPlayerRenderer playerrenderer = (NewPlayerRenderer) this.entityRenderDispatcher.<AbstractClientPlayerEntity>getRenderer(this.minecraft.player);
    ...
    private void renderPlayerArm(MatrixStack p_228401_1_, IRenderTypeBuffer p_228401_2_, int p_228401_3_, float p_228401_4_, float p_228401_5_, HandSide p_228401_6_) {
        ...
        NewPlayerRenderer playerrenderer = (NewPlayerRenderer) this.entityRenderDispatcher.<AbstractClientPlayerEntity>getRenderer(abstractclientplayerentity);
        if (flag) {
    ...
    private int verify(int i, int max) {
        return Math.min(i, max);
    }

    private void applyCutlassSwingTransform(MatrixStack poseStack, float swingProgress, HandSide handSide, float equipProgress) {
        this.applyItemArmTransform(poseStack, handSide, equipProgress);

        float speed = 0.6f;
        float swing = 1.0f - MathHelper.clamp(swingProgress * speed, 0.0f, 1.0f);
        float swingRadians = swing * 2.0f * (float) Math.PI;
        float rotation = -15.0f - 75.0f * MathHelper.cos(swingRadians);

        // Animate Y and Z positions
        float yAnim = MathHelper.lerp(swingProgress, 0.65f, 0.85f); // from 0.5 to 0.8
        float zAnim = MathHelper.lerp(swingProgress, -0.45f, 0.35f); // from farther (0.6) to closer (0.35)

        if (handSide != HandSide.RIGHT) {
            poseStack.translate(0.1, yAnim, zAnim);
    ...
    private void applyBrushTransform(MatrixStack poseStack, float f, HandSide humanoidArm, ItemStack itemStack, float f2) {
        this.applyItemArmTransform(poseStack, humanoidArm, f2);
        float f3 = this.minecraft.player.getUseItemRemainingTicks() % (10 - (verify(EnchantmentHelper.getItemEnchantmentLevel(Enchantments.EXCAVATION, itemStack), 1)));
        float f4 = f3 - f + 1.0f;
        ...
        float f9 = -15.0f;
        int n = 2;
        float f10 = -15.0f + 75.0f * MathHelper.cos(f5 * 2.0f * (float) Math.PI);
        if (humanoidArm != HandSide.RIGHT) {
    ...
    public void renderHandsWithItems(float p_228396_1_, MatrixStack p_228396_2_, IRenderTypeBuffer.Impl p_228396_3_, ClientPlayerEntity p_228396_4_, int p_228396_5_) {
        ...
        if (p_228396_4_.isUsingItem()) {
            ...
            if (itemstack.getItem() == Items.BOW || itemstack.getItem() == Items.CROSSBOW || itemstack.getItem() == Items.BONE_BOW || itemstack.getItem() == Items.GILDED_CROSSBOW || itemstack.getItem() instanceof AbstractCrossbowItem || itemstack.getItem() instanceof TerrariaBowItem) {
    ...
    private void renderArmWithItem(AbstractClientPlayerEntity p_228405_1_, float p_228405_2_, float p_228405_3_, Hand p_228405_4_, float p_228405_5_, ItemStack handItem, float p_228405_7_, MatrixStack p_228405_8_, IRenderTypeBuffer p_228405_9_, int p_228405_10_) {
        if (p_228405_1_.isScoping()) return;
        ...
        if (handItem.isEmpty()) {
            ...
            if (p_228405_1_.isUsingItem() && p_228405_1_.getUseItemRemainingTicks() > 0 && p_228405_1_.getUsedItemHand() == p_228405_4_) {
                ...
                switch (handItem.getUseAnimation()) {
                    case BRUSH:
                        applyBrushTransform(p_228405_8_, p_228405_2_, handside, handItem, p_228405_7_);
                        break;
                    case NONE:
                        this.applyItemArmTransform(p_228405_8_, handside, p_228405_7_);
                        break;
                ...
                if (handItem.getItem() == Items.WITHER_BONE_CUTLASS && p_228405_5_ != 0.0F && (minecraft.crosshairPickEntity != null || minecraft.hitResult != null && minecraft.hitResult.getType() != RayTraceResult.Type.BLOCK)) {
                    applyCutlassSwingTransform(p_228405_8_, p_228405_5_, handside, p_228405_7_);
                } else {
                    // Default sword animation

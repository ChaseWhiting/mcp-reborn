   ...
   public int retrieve(ItemStack item) {
      ...
      if (!this.level.isClientSide && playerentity != null) {
         ...
         if (this.hookedIn != null) {
            ...
            CriteriaTriggers.FISHING_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, item, this, Collections.emptyList());
            ...
            if (this.random.nextFloat() < 0.75) {
               ...
               if (this.random.nextFloat() < 0.2) {
                  entityToSpawn = new CodEntity(EntityType.COD, this.level);
               } else if (this.random.nextFloat() < 0.4) {
                  entityToSpawn = new SalmonEntity(EntityType.SALMON, this.level);
               } else if (this.random.nextFloat() < 0.08) {
                  entityToSpawn = new SquidEntity(EntityType.SQUID, this.level);
               } else if (this.random.nextFloat() < 0.4) {
                  entityToSpawn = new TropicalFishEntity(EntityType.TROPICAL_FISH, this.level);
                  if (entityToSpawn instanceof  TropicalFishEntity) {
                     TropicalFishEntity fish = (TropicalFishEntity) entityToSpawn;
                     fish.setRandomColor();
                  }
               } else if (this.random.nextFloat() < 0.2) {
                  entityToSpawn = new GuardianEntity(EntityType.GUARDIAN, this.level);
               } else if (this.random.nextFloat() < 0.07) {
                  entityToSpawn = new DolphinEntity(EntityType.DOLPHIN, this.level);
               } else if (this.random.nextFloat() < 0.02){
                  entityToSpawn = new ElderGuardianEntity(EntityType.ELDER_GUARDIAN, this.level);
               }

               if (entityToSpawn != null) {
                  // fix squid pos
                  if (entityToSpawn instanceof SquidEntity) {
                     entityToSpawn.setPos(this.getX() , this.getY() + 1.2, this.getZ());
                  } else
                     entityToSpawn.setPos(this.getX(), this.getY(), this.getZ());

                   // casting entity
                  if (entityToSpawn instanceof LivingEntity) {
                     LivingEntity entity = (LivingEntity) entityToSpawn;
                     float maxHealth = entity.getMaxHealth();
                     if (!(entity instanceof  ElderGuardianEntity)) {
                        entity.setHealth(maxHealth / 2.0F);
                     }
                  }
                  ...
                  entityToSpawn.setDeltaMovement(d0 * 0.1D, d1 * 0.12D + Math.sqrt(Math.sqrt(d0 * d0 + d1 * d1 + d2 * d2)) * 0.08D, d2 * 0.1D);

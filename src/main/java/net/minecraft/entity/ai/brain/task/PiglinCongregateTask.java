...
public class PiglinCongregateTask extends Task<LivingEntity> {
   public PiglinCongregateTask() {
   ...
   protected boolean checkExtraStartConditions(ServerWorld p_212832_1_, LivingEntity p_212832_2_) {
      ...
      return p_212832_1_.getRandom().nextInt(100) == 0 && optional.isPresent() && p_212832_1_.dimension() == optional.get().dimension() && optional.get().pos().closerThan(p_212832_2_.position(), 4.0D) && brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS).get().stream().anyMatch((p_220570_0_) -> {
         return EntityType.PIGLIN.equals(p_220570_0_.getType());
      });
   }

   protected void start(ServerWorld p_212831_1_, LivingEntity p_212831_2_, long p_212831_3_) {
      Brain<?> brain = p_212831_2_.getBrain();
      brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS).ifPresent((p_220568_2_) -> {
         p_220568_2_.stream().filter((p_220572_0_) -> {
            return EntityType.PIGLIN.equals(p_220572_0_.getType());
         }).filter((p_220571_1_) -> {

...
public class AttackStrafingTask<E extends MobEntity> extends Task<E> {
   ...
   public AttackStrafingTask(int tooCloseDistance, float strafeSpeed, int mobCloseDistance) {
   ...
   private float determineStrafeDirection(E entity) {
      ...
      for (MobEntity mob : nearbyMobs) {
         if (mob != entity && mob.getClass() == entity.getClass()) {
            // Determine strafe direction to avoid the nearby mob
            // If the nearby mob is on the left side, strafe to the right (+1.0F), and vice versa
            double deltaX = mob.getX() - entity.getX();
            return deltaX > 0 ? 1.0F : -1.0F;
         }
      }

...
public class BoggedAttackGoal<T extends Monster & IRangedAttackMob> extends Goal {
    ...
    public BoggedAttackGoal(T p_i47515_1_, double p_i47515_2_, int p_i47515_4_, float p_i47515_5_) {
    ...
    public void tick() {
        ...
        if (livingentity != null) {
            ...
            if (this.strafingTime >= 15) {
                this.strafingClockwise = this.mob.getRandom().nextFloat() < 0.4F;  // More random strafing
                this.strafingBackwards = false;  // No jumping, only strafing forward/backward
                this.strafingTime = 0;
            }
            ...
            if (this.strafingTime > -1) {
                ...
                strafeSpeed = d0 > this.attackRadiusSqr * 0.5F ? 0.7F : 0.9F;
                ...
                if (this.strafingBackwards) {
                    ...
                    if (this.mob.nextFloat() < (isClose ? 0.02 : 0.008)) {
...
//                if (targetEntity.invulnerableTime > 0 && targetEntity.getHealth() >= 5 && !(targetEntity instanceof PlayerEntity)) {
//                    canShoot = false;
//                    if (targetEntity instanceof IronGolemEntity) {
//                        this.strafeSpeed = 1.22F;
//                        this.mob.strafe(-strafeSpeed, this.mob.nextBoolean() ? -strafeSpeed : strafeSpeed);
//                    }
//                }
    ...
    private boolean anyBlocked(LivingEntity livingentity, LivingEntity targetEntity) {
        ...
        for (LivingEntity nearbyEntity : this.mob.level.getEntitiesOfClass(LivingEntity.class, lineOfSightArea)) {
            if (nearbyEntity instanceof Mob) {
                if (nearbyEntity.as(Mob.class).getTarget() == this.mob) {
                    canShoot = true;
                    return canShoot;
                }
            }

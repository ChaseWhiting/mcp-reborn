...
public abstract class CustomInteractDoorGoal extends Goal {
   protected Mob mob;
   ...
   public CustomInteractDoorGoal(Mob p_i1621_1_) {
      this.mob = p_i1621_1_;
      ...
      if (!GroundPathHelper.hasGroundPathNavigation(this.mob)) {
         ...
         if (path != null && !path.isDone() && groundpathnavigator.canOpenDoors()) {
            for(int i = 0; i < Math.min(path.getNextNodeIndex() + 3, path.getNodeCount()); ++i) {
               ...
               if (!(this.mob.distanceToSqr((double)this.doorPos.getX(), this.mob.getY(), (double)this.doorPos.getZ()) > 3.3D)) {

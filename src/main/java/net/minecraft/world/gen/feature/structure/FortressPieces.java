...
public class FortressPieces {
    ...
    public static class End extends FortressPieces.Piece {
        ...
        protected void generateBox(ISeedReader world, MutableBoundingBox box,
                                   int minX, int minY, int minZ,
                                   int maxX, int maxY, int maxZ,
                                   BlockState outerBlock, BlockState innerBlock,
                                   boolean replaceNonAirOnly) {

            for (int y = minY; y <= maxY; ++y) {
                for (int x = minX; x <= maxX; ++x) {
                    for (int z = minZ; z <= maxZ; ++z) {
                        if (!replaceNonAirOnly || !this.getBlock(world, x, y, z, box).isAir()) {

                            boolean isEdge = (y == minY || y == maxY ||
                                    x == minX || x == maxX ||
                                    z == minZ || z == maxZ);

                            this.placeBlock(world, isEdge ? outerBlock : innerBlock, x, y, z, box);

                            if (isEdge) {
                                this.potentialSuspiciousCrackedBricksWorldPositions.add(new BlockPos(this.getWorldX(x, z), this.getWorldY(y), this.getWorldZ(x, z)));
                            } else {
        ...
        public double getCrackedBlockPlaceChance() {
            return 0.4D;
        }
                ...
                for (int x = minX; x <= maxX; ++x) {
                    for (int z = minZ; z <= maxZ; ++z) {
                        if (!replaceNonAirOnly || !this.getBlock(world, x, y, z, box).isAir()) {
                            ...
                            if (minX == 5 && minY == 5 && minZ == 5 && maxX == 7 && maxZ == 7) {
                                if (world.getRandom().nextFloat() < 0.85F) {
                                    this.potentialSuspiciousCrackedBricksWorldPositions.add(new BlockPos(this.getWorldX(x, z), this.getWorldY(y), this.getWorldZ(x, z)));
                                }
                            }
                        }
    ...
    abstract static class Piece extends StructurePiece {
        ...
        public final List<BlockPos> potentialSuspiciousCrackedBricksWorldPositions = new ArrayList<>();

        public List<BlockPos> getPotentialSuspiciousCrackedBricksWorldPositions() {
            return potentialSuspiciousCrackedBricksWorldPositions;
        }
        ...
        protected void fillColumnDown(ISeedReader p_175808_1_, BlockState p_175808_2_, int p_175808_3_, int p_175808_4_, int p_175808_5_, MutableBoundingBox p_175808_6_) {
            ...
            if (p_175808_6_.isInside(new BlockPos(i, j, k))) {
                while ((p_175808_1_.isEmptyBlock(new BlockPos(i, j, k)) || p_175808_1_.getBlockState(new BlockPos(i, j, k)).getMaterial().isLiquid()) && j > 1) {
                    ...
                    if (p_175808_1_.getRandom().nextFloat() < 0.45F) {
                        this.potentialSuspiciousCrackedBricksWorldPositions.add(new BlockPos(i, j, k));
                    }
                ...
                for (int x = minX; x <= maxX; ++x) {
                    for (int z = minZ; z <= maxZ; ++z) {
                        if (!replaceNonAirOnly || !this.getBlock(world, x, y, z, box).isAir()) {
                            ...
                            if (outerBlock.getBlock() == Blocks.NETHER_BRICKS || innerBlock.getBlock() == Blocks.NETHER_BRICKS) {
                                if (isEdge) {
                                    if (world.getRandom().nextFloat() < 0.1F) {
                                        this.potentialSuspiciousCrackedBricksWorldPositions.add(new BlockPos(this.getWorldX(x, z), this.getWorldY(y), this.getWorldZ(x, z)));
                                    }

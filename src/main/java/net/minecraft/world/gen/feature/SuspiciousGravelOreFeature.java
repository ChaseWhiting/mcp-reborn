...
public class SuspiciousGravelOreFeature extends Feature<OreFeatureConfig> {
   public SuspiciousGravelOreFeature(Codec<OreFeatureConfig> p_i231976_1_) {
      super(p_i231976_1_);
   }
                           ...
                           if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                              ...
                              if (!bitset.get(l2)) {
                                 ...
                                 if (p_207803_3_.target.test(p_207803_1_.getBlockState(blockpos$mutable), p_207803_2_)) {
                                    ...
                                    if (p_207803_1_.getBlockEntity(blockpos$mutable.immutable()) instanceof BrushableBlockEntity brushableBlock) {
                                        if (p_207803_2_.nextFloat() < 0.12F) {
                                            brushableBlock.setLootTable(LootTables.GRAVEL_ORE_RARE_ARCHAEOLOGY, RandomSource.create(p_207803_1_ instanceof ServerWorld serverWorld ? serverWorld.getSeed() : p_207803_2_.nextLong()).forkPositional().at(blockpos$mutable).nextLong());
                                        } else {
                                           brushableBlock.setLootTable(LootTables.GRAVEL_ORE_COMMON_ARCHAEOLOGY, RandomSource.create(p_207803_1_ instanceof ServerWorld serverWorld ? serverWorld.getSeed() : p_207803_2_.nextLong()).forkPositional().at(blockpos$mutable).nextLong() >> 8);
                                        }
                                    }

...
public class HugeCanyonWorldCarver extends WorldCarver<ProbabilityConfig> {
   private final float[] rs = new float[1024];

   public HugeCanyonWorldCarver(Codec<ProbabilityConfig> p_i231916_1_) {
   ...
   public boolean carve(IChunk p_225555_1_, Function<BlockPos, Biome> p_225555_2_, Random p_225555_3_, int p_225555_4_, int p_225555_5_, int p_225555_6_, int p_225555_7_, int p_225555_8_, BitSet p_225555_9_, ProbabilityConfig p_225555_10_) {
      ...
      // Keep height within 7-15 blocks and above Y=20
      double d1 = (double)(p_225555_3_.nextInt(8) + 7) + 20; // 7-15 blocks above Y=20
      ...
      // Make caves wider and shorter
      float f2 = (p_225555_3_.nextFloat() * 2.5F + p_225555_3_.nextFloat()) * 3.0F; // Wider

      int j = i / 2 - p_225555_3_.nextInt(i / 4); // Shorter in length
      int k = 0;

      this.genCanyon(p_225555_1_, p_225555_2_, p_225555_3_.nextLong(), p_225555_4_, p_225555_7_, p_225555_8_, d0, d1, d2, f2, f, f1, 0, j, 2.5D, p_225555_9_); // Slightly more round
      return true;
   }
   ...
   private void genCanyon(IChunk p_227204_1_, Function<BlockPos, Biome> p_227204_2_, long p_227204_3_, int p_227204_5_, int p_227204_6_, int p_227204_7_, double p_227204_8_, double p_227204_10_, double p_227204_12_, float p_227204_14_, float p_227204_15_, float p_227204_16_, int p_227204_17_, int p_227204_18_, double p_227204_19_, BitSet p_227204_21_) {
      ...
      for (int j = p_227204_17_; j < p_227204_18_; ++j) {
         double d0 = 2.5D + (double)(MathHelper.sin((float) j * (float) Math.PI / (float) p_227204_18_) * p_227204_14_);
         double d1 = d0 * p_227204_19_;

         // Reduce overall height variation for rounded caves
         d0 = d0 * ((double) random.nextFloat() * 0.2D + 0.8D);
         d1 = d1 * ((double) random.nextFloat() * 0.2D + 0.8D);

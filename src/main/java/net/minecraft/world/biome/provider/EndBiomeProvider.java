...
public class EndBiomeProvider extends BiomeProvider {
   ...
   public static float getHeightValue(SimplexNoiseGenerator p_235317_0_, int p_235317_1_, int p_235317_2_) {
      ...
      int k = p_235317_1_ % 2;
      int l = p_235317_2_ % 2;
      float f = 120.0F - MathHelper.sqrt((float)(p_235317_1_ * p_235317_1_ + p_235317_2_ * p_235317_2_)) * 10.0F; // Increase height variability
      f = MathHelper.clamp(f, -120.0F, 100.0F); // Increase clamp range

      for(int i1 = -16; i1 <= 16; ++i1) { // Increase noise detail range
         for(int j1 = -16; j1 <= 16; ++j1) {
            long k1 = (long)(i + i1);
            long l1 = (long)(j + j1);
            if (k1 * k1 + l1 * l1 > 4096L && p_235317_0_.getValue((double)k1 * 0.5D, (double)l1 * 0.5D) < (double)-0.9F) { // Adjust noise frequency
               ...
               float f4 = 120.0F - MathHelper.sqrt(f2 * f2 + f3 * f3) * f1;
               f4 = MathHelper.clamp(f4, -120.0F, 100.0F);
               f = Math.max(f, f4);
            }
         }
      }
